...
class SEPC(nn.Module):
    def __init__(
        ...
        self.lconv1 = nn.Sequential(
            nn.Conv2d(out_channels, down_channels, 1),
            nn.ReLU(),
            nn.Conv2d(down_channels,
                   down_channels,
                   kernel_size=3,
                   dilation=1,
                   padding=1),
            nn.ReLU(),
            nn.Conv2d(down_channels, out_channels, 1),
        )
        """
        self.lconv1 = nn.Conv2d(down_channels,
                   down_channels,
                   kernel_size=3,
                   dilation=1,
                   padding=1)
        self.lconv2 = nn.Conv2d(down_channels,
                   down_channels,
                   kernel_size=3,
                   dilation=2,
                   padding=2)

        self.lconv3 = nn.Conv2d(down_channels,
                   down_channels,
                   kernel_size=3,
                   dilation=3,
                   padding=3)

        self.lconvd = nn.Conv2d(out_channels * 4,
                                out_channels,
                                kernel_size=1,)
        self.relu = nn.ReLU()
        if self.iBN:
    ...
    def init_weights(self):
        for str in ['conv1', 'conv2', 'conv3', 'convd']:#, 'conv4'
    ...
    def forward(self, inputs):
        ...
        loc1 = [self.lconv1(level) for level in x]
        loc2 = [self.lconv2(level) for level in x]
        loc3 = [self.lconv3(level) for level in x]

        loc=[]
        for i, j, k, l in zip(loc1, loc2, loc3, x):
            loc.append(torch.cat([i, j, k, l], dim=1))

        x = [self.lconvd(level) for level in loc]

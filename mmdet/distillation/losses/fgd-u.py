...
class FeatureLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.dss = nn.Conv2d(teacher_channels*4, teacher_channels, kernel_size=1,)
        ...
        self.scale_t3 = nn.Conv2d(teacher_channels, teacher_channels, kernel_size=3,
                     dilation=3, padding=3)
        self.dst = nn.Conv2d(teacher_channels * 4, teacher_channels, kernel_size=1,)
    ...
    def forward(self,
       ...
       # losses.update({"loss_uncertainty_cls": uncertainty_prediction_cls.sum() / uncertainty_prediction_cls.numel() * self.uncertainty_cls_weight})
        ...
        loss =  self.beta_fgd * bg_loss + self.alpha_fgd * fg_loss +  self.gamma_fgd * mask_loss  + self.lambda_fgd * rela_loss
